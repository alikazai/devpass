...
const ManageApp = (): JSX.Element => {
  ...
  const refreshStatus = async (): Promise<void> => {
    const response = await sendMessage<StatusResponse['data']>({ type: 'vault:status' });
    if (response.ok) {
  ...
  return (
    <div className="container">
      <header className="header">
        <div>
          <h1>DevPass Manager</h1>
          <p className="muted">Manage projects, environments, and accounts.</p>
        </div>
      ...
      <section className="panel">
        ...
        <div className="grid">
          ...
          <input
            ...
            onChange={(event) =>
              editingAccountId
                ? setEditAccountSecret(event.target.value)
